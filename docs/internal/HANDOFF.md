...
**Version:** v0.15.0 (dashboard polish + functional Share menu + connect flow + Profile persistence + theme toggle; pending commit)
**Last committed version:** v0.14.0 (report aesthetic rework + dashboard upgrade + Google Ads + report history)
...
- **v0.15.0** (pending commit). Polish wave on top of v0.14.0. (a) Stripped every user-facing "Claude" mention so the brain stays under the hood. (b) Range tabs + section header moved above Traffic Overview so they scope everything below; title is range-aware. (c) Live pulse badge on TrafficOverviewCard. (d) Per-card `<CardManageAction>` (Connect / Connect live / Reconnect) + dashed `<AddFeedTile>` at the end of the grid; click-through to a new connect picker (`/app/businesses/[slug]/connect`) and per-integration page (`/app/businesses/[slug]/connect/[integration]`) with side-by-side Live/Manual cards. (e) Create Report opens in a new tab via `window.open` (Server Action returns `{ shareToken, shareUrl }` instead of redirecting). (f) Share Report menu functional: Copy share link looks up the most recent report token via `latestShareTokenForBusiness` and copies the share URL; Download PDF opens the report with `?print=1` which auto-fires `window.print()` (interim until react-pdf template lands); Email kept as "Soon" honestly. (g) Theme toggle on the public report page so recipients can flip light/dark. (h) Edit Profile persists via `saveBusinessProfile` Server Action — name, shortName, tagline, vertical, brandColor, logoUrl all write to the `businesses` table; logo file-upload still says "Soon" (paste URL works in the meantime). (i) Typeform table reshaped: dropped email, added Date / Name / Company / "What they need" / Status; extractor pulls `company` and `message` (longest narrative answer).
